...
- **AND** hook 命令应当是 ccc 的绝对路径加 `supervisor-hook --state-dir .claude/ccc`

#### Scenario: Hook 命令格式
- **GIVEN** ccc 安装在 `/usr/local/bin/ccc`
- **WHEN** 系统生成 hook 配置
- **THEN** hook 命令应当为 `/usr/local/bin/ccc supervisor-hook --state-dir .claude/ccc`
...
- **WHEN** 执行 `ccc supervisor-hook --state-dir .claude/ccc`
...
- **THEN** 应当输出 "[ccc supervisor-hook] Skipping (CCC_SUPERVISOR_HOOK=1), allowing stop" 到 stderr
- **AND** 应当输出空内容到 stdout
- **AND** 应当立即返回（退出码 0）
...
- **THEN** 命令应当包含：
  ...
  - `--system-prompt` （SUPERVISOR.md 内容）
...
- **AND** 应当将原始输出保存到 `.claude/ccc/supervisor-{session_id}-output.jsonl`
