...
- **THEN** 应当输出 "Provider 'glm': Invalid"
- **AND** 显示具体错误 "Missing required environment variable: ANTHROPIC_AUTH_TOKEN"

#### Scenario: 验证无效的 Base URL
- **GIVEN** ccc.json 中配置了 `test` 提供商
- **AND** `ANTHROPIC_BASE_URL` 格式无效（如 "not-a-url"）
- **WHEN** 用户执行 `ccc validate test`
- **THEN** 应当输出 "Provider 'test': Invalid"
...
- **THEN** 应当输出 "Provider 'offline': Warning - API connection failed"
- **AND** 显示连接错误详情
...
- **THEN** 应当输出汇总信息 "Validating 3 providers..."
- **AND** 显示每个提供商的验证结果
...
- **THEN** 应当输出 "Config file is invalid"
- **AND** 显示具体的 JSON 解析错误
- **AND** 返回非零退出码

#### Scenario: 配置文件缺少必需字段
- **GIVEN** ccc.json 缺少 `providers` 字段
- **WHEN** 用户执行 `ccc validate`
- **THEN** 应当输出 "Config is missing required field: providers"
- **AND** 返回非零退出码
...
- **WHEN** 验证失败
- **THEN** 应当在错误信息后提供修复建议
- **EXAMPLE**: "Missing ANTHROPIC_AUTH_TOKEN. Add it to your provider config: ccc.json"
...
- **GIVEN** 有效提供商配置
- **WHEN** 执行 API 连通性测试
- **THEN** 应当向 `{BASE_URL}/v1/messages` 发送轻量级请求
- **AND** 根据响应判断连接是否成功
...
- **AND** 显示 "API connection: Failed - Invalid API token"

#### Scenario: 跳过 API 测试
- **WHEN** 用户使用 `--no-api-test` 参数
- **THEN** 应当跳过 API 连通性测试
- **AND** 只验证配置格式和环境变量完整性
